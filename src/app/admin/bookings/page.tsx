...
async function getBookings() {
  ...
  return {
    carWash: (carWash ?? []) as unknown as CarWashRow[],
    workshop: (workshop ?? []) as unknown as WorkshopRow[],
  };
}
      ...
      <section className="mb-12">
        ...
        <div className="grid gap-3">
          {carWash.map((b) => (
            <div
              key={b.id}
              className="border border-[color:var(--line-dark)] rounded-[4px] p-5 bg-bg"
            >
              <div className="flex items-start justify-between gap-4">
                <div>
                  ...
                  <div className="text-xs text-muted mt-1">
                    {b.address}
                    {b.requested_date ? ` · gewenst: ${b.requested_date}` : ""}
                  </div>
                </div>
                <LeadStatusSelect
                  id={b.id}
          ...
          {carWash.length === 0 && (
            <p className="text-muted text-sm font-mono">Nog geen aanvragen.</p>
          )}
        </div>
      </section>

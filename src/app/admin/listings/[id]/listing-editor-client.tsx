...
export function DossierPhotoUpload({ eventId, listingId }: { eventId: string; listingId: string }) {
  const [isPending, startTransition] = useTransition();
  const formRef = useRef<HTMLFormElement>(null);
  ...
  return (
    <form
      ...
      action={(formData) =>
        startTransition(async () => {
          await uploadDossierPhoto(eventId, listingId, formData);
          formRef.current?.reset();
        })
      }
    ...
    >
      <input
        ...
        name="photo"
        accept="image/*"
        className="text-[11px] text-muted font-mono file:mr-2 file:py-1.5 file:px-2.5 file:rounded-[3px] file:border-0 file:bg-bg-soft-2 file:text-paper file:text-[11px] file:font-mono"
      />
      <button type="submit" disabled={isPending} className="btn btn-ghost text-[11px] py-1.5 px-3 disabled:opacity-60">
        {isPending ? 'Uploaden…' : '+ Foto'}
      </button>
    </form>
  );
}

export function DeleteDossierPhotoButton({ photoId, listingId }: { photoId: string; listingId: string }) {
  const [isPending, startTransition] = useTransition();
  return (
    <button
      type="button"
      disabled={isPending}
      onClick={() => startTransition(() => deleteDossierPhoto(photoId, listingId))}
      className="absolute top-1 right-1 bg-bg/80 text-orange-bright text-[9px] font-mono px-1 py-0.5 rounded-[2px]"
    >
      ✕
    </button>
  );

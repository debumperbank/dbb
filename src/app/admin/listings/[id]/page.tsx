...
async function getListing(id: string) {
  ...
  if (eventIds.length > 0) {
    const { data } = await supabase
      ...
      .order("sort_order");
    dossierPhotos = (data ?? []) as unknown as RestorationEventPhoto[];
  }

  return {
    listing: listing as unknown as ListingWithCar,
    photos: (photos ?? []) as Photo[],
    history: (history ?? []) as unknown as RestorationEvent[],
    dossierPhotos,
  };
}
  ...
  return (
    <div className="max-w-2xl">
      ...
      <p className="text-muted font-mono text-sm mb-8">
        {car.build_year} &middot; {listing.department} &middot;{" "}
        {formatPriceCents(listing.price_cents)} &middot; status:{" "}
        {listing.status}
      </p>
      ...
      <section className="mb-10">
        ...
        <div className="grid grid-cols-3 sm:grid-cols-4 gap-3 mb-4">
          {photos.map((p) => (
            <div
              key={p.id}
              className="relative aspect-square rounded-[3px] overflow-hidden border border-[color:var(--line-dark)]"
            >
      ...
      {car.is_oldtimer && (
        <section>
          ...
          <div className="grid gap-5 mb-2">
            {history.map((ev) => {
              const evPhotos = dossierPhotos.filter(
                (p) => p.restoration_event_id === ev.id,
              );
              return (
                <div
                  key={ev.id}
                  className="border-l border-[color:var(--line-dark)] pl-4"
                >

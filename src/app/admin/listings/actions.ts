...
export async function createListing(formData: FormData) {
  ...
  const mileageKm = formData.get("mileage_km")
  ...
  const slug = `${slugBase}-${String(car.id).slice(0, 6)}`;
...
export async function addRestorationEvent(
  carId: string,
  listingId: string,
  formData: FormData,
) {
  ...
  revalidatePath("/occasions/[id]", "page");
  revalidatePath("/voorraad/[id]", "page");
...
export async function uploadDossierPhoto(
  ...
  await supabase.from("restoration_event_photos").insert({
    restoration_event_id: eventId,
    url: publicUrl.publicUrl,
    sort_order: 0,
  });
  ...
  revalidatePath("/voorraad/[id]", "page");
  revalidatePath(`/admin/listings/${listingId}`);
  revalidatePath(`/voorraad/${listingId}`);
}

export async function deleteDossierPhoto(photoId: string, listingId: string) {
  const supabase = await crmClient();
  await supabase.from("restoration_event_photos").delete().eq("id", photoId);
  revalidatePath("/occasions/[id]", "page");
  revalidatePath("/voorraad/[id]", "page");
  revalidatePath(`/admin/listings/${listingId}`);
  revalidatePath(`/voorraad/${listingId}`);

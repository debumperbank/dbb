...
async function getRestorationHistory(carId: string): Promise<{
  ...
  const { data: photos } = await supabase
    ...
    .in(
      ...
      (events as unknown as RestorationEvent[]).map((e) => e.id),
    )
    .order("sort_order");
...
export default async function CarDetailPage({
  ...
  const { data: listingPhotos } = await db
    .from("listing_photos")
    .select("id,url")
      ...
      <div className="max-w-site mx-auto grid md:grid-cols-[1.2fr_0.8fr] gap-14">
        <div>
          <div className="eyebrow">
            <span className="dot" />
            {car.build_year} &middot; {listing.department}
          </div>
          <h1 className="mt-3 text-3xl md:text-4xl">
            {car.make} {car.model}
          </h1>
          <div className="mt-5 font-display text-2xl text-orange">
            {formatPriceCents(listing.price_cents)}
          </div>

          {!!listingPhotos?.length && (
            <div className="grid grid-cols-2 gap-3 mt-7">
              {(listingPhotos as { id: string; url: string }[]).map((photo) => (
                <a
                  key={photo.id}
                  href={photo.url}
                  target="_blank"
          ...
          <dl className="mt-8 grid grid-cols-2 gap-5 font-mono text-sm">
            <div>
              <dt className="text-muted text-[11px] uppercase">
                Kilometerstand
              </dt>
          ...
          {car.description && (
            <p className="mt-8 text-[15px] leading-relaxed text-muted max-w-[60ch]">
              {car.description}
            </p>
          )}
                      ...
                      {evPhotos.length > 0 && (
                        <div className="grid grid-cols-3 sm:grid-cols-4 gap-2 mt-3 max-w-md">
                          {evPhotos.map((p) => (
                            <div
                              key={p.id}
                              className="aspect-square rounded-[3px] overflow-hidden border border-[color:var(--line-dark)]"
                            >
